...
type Tab = "overview" | "ads" | "campaigns" | "adsets" | "books" | "funnel" | "compare" | "mapping" | "imports";
...
export default function AdsPage() {
  ...
  const showFilters = tab !== "mapping" && tab !== "imports" && tab !== "compare";
          ...
          {showFilters && (
            <div className="card mb-5 p-4">
              <div className="flex flex-wrap items-center gap-2">
                <MultiSelect
                  options={periods.map((p) => p.period_key)}
                  values={selPeriods}
                  onChange={setSelPeriods}
                  placeholder={tx(AD.period)}
                  getLabel={(v) => periods.find((p) => p.period_key === v)?.period_label ?? v}
                ...
                <table className="table-base">
                  ...
                  <tbody>
                    {bookRows.map((g) => {
                      ...
                      return (
                        <tr key={g.key}>
                          <td className="!whitespace-normal max-w-[220px] font-medium">{g.key}</td>
