...
import { Spinner, EmptyState, SortTh, useSort } from "@/components/ui";
import { formatNumber, toCsv, downloadCsv, cn } from "@/lib/utils";

/* ------------------------------------------------------------------ *
 * Demand forecast — was its own /forecast page. It answers the same
 * question as the rest of the Stock page ("what do I need to buy?"),
 * just from velocity + seasonality instead of current stock levels,
 * so it lives here as a tab and the two can be read side by side.
 * ------------------------------------------------------------------ */
  ...
  return (
    <div>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-2 rounded-lg border border-brand-100 bg-brand-50 px-4 py-2.5 text-xs text-brand-800">
              ...
              {sortedRows.map((r) => (
                <tr key={r.sku}>
                  <td className="!whitespace-normal max-w-xs font-medium">
                    <a href={`/products?q=${encodeURIComponent(r.sku)}`} className="text-brand-700 hover:underline">
                      {r.product_name}
                    </a>
                  </td>
                  <td dir="ltr" className="font-mono text-xs text-slate-500">
                    {r.sku}
                  </td>
                  ...
                  <td>
                    <span
                      className={cn(
                        "font-semibold",
                        r.seasonal_factor > 1.2
                          ? "text-emerald-600"
                          : r.seasonal_factor < 0.8
                            ? "text-red-600"
                            : "text-slate-600"
                      )}
                    >
                  ...
                  <td
                    className={cn(
                      r.current_ecom != null && r.current_ecom < r.forecast_next_30 && "font-semibold text-red-600"
                    )}
                  >
                    {r.current_ecom != null ? formatNumber(r.current_ecom) : "—"}
                  </td>
                  <td className={cn("font-bold", r.suggested_buy > 0 ? "text-amber-700" : "text-slate-400")}>
                    {formatNumber(r.suggested_buy)}
                  </td>
                </tr>
              ))}
            </tbody>
