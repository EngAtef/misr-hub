-- ============================================================
-- Migration 149: fn_traffic_alarms runs as SECURITY DEFINER.
--
-- The alerts bar on every page calls fn_traffic_alarms(). As a plain
-- `language sql` invoker function it ran under RLS for the signed-in user:
-- 10.4 s as `authenticated` vs 1.3 s as postgres (measured 2026-09-15),
-- so it hit the 8 s statement_timeout and the bar logged a 500 on load.
-- Same fix as migrations 135-137 / 142: plpgsql, security definer, and the
-- reader gate (fn_assert_reader) as the first statement. Body unchanged.
-- ============================================================
...
$function$;
revoke execute on function public.fn_traffic_alarms() from public, anon;
grant execute on function public.fn_traffic_alarms() to authenticated, service_role;
