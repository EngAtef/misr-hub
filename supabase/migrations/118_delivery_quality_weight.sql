-- ============================================================
-- Migration 118: weight on the delivery-quality report.
-- fn_delivery_quality summary + by_city gain weight_kg (non-cancelled),
-- delivered_weight_kg / returned_weight_kg (returned or RTS — the weight
-- that travelled both ways). Body otherwise identical to 068.
-- Run after 117_vendor_customer_weight.sql
-- ============================================================
  ...
  select jsonb_build_object(
    'summary', (
      select jsonb_build_object(
        ...
        'weight_kg',     round(coalesce(sum(weight_kg) filter (where not is_cancelled), 0), 2),
        'delivered_weight_kg', round(coalesce(sum(weight_kg) filter (where is_delivered), 0), 2),
        'returned_weight_kg',  round(coalesce(sum(weight_kg) filter (where is_returned or is_rts), 0), 2)
      )
      from base
    ),
    ...
    'by_city', coalesce((
      ...
      from (
        select
          ...
          round(coalesce(sum(weight_kg) filter (where not is_cancelled), 0), 2) as weight_kg,
          round(coalesce(sum(weight_kg) filter (where is_returned or is_rts), 0), 2) as returned_weight_kg
        from base
        group by city
