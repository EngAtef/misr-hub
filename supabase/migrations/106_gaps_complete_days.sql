-- ============================================================
-- Migration 106: GAPS compares complete days only.
--
-- Today is never a fair comparison: GA4 is pulled once at ~04:30 UTC (a
-- few hours of the day) while orders import all day, so the current month
-- always showed a phantom gap for "today" and a lower tracking rate. Every
-- running-month figure — orders, GA4 daily/sources, GSC, the order↔GA4
-- transaction match, day and week series — now stops at the last complete
-- day (yesterday). Closed months are unchanged. Freshness reports the last
-- COMPLETE GA4/GSC day plus the real last sync time.
-- ============================================================
...
as $$
  with bounds as (
    select p_month as m_from,
           least((p_month + interval '1 month')::date, current_date) as m_to
  ),
  o as (
    select order_number, total_order_amount, source, payment_method
    from public.orders, bounds b
    where order_date >= b.m_from and order_date < b.m_to
  ),
  g as (
    ...
    where t.period_month = p_month
      and not exists (select 1 from public.orders x
                      where x.order_number = t.transaction_id and x.order_date >= b.m_to)
  )
...
as $$
with bounds as (
  -- a running month stops at the last COMPLETE day: today is only half
  -- synced in GA4 (04:30 UTC pull) while orders are live, so it can never
  -- be compared fairly. Closed months are unaffected.
...
tx as (
  ...
  where t.period_month = b.m_from
    -- a transaction whose order falls after the cut (today) is out too
    and not exists (select 1 from public.orders x
                    where x.order_number = t.transaction_id and x.order_date >= b.m_to)
),
buckets as (
...
-- ------------------------------------------------------- day-by-day view
days as (
  select generate_series(b.m_from, b.m_to - 1, interval '1 day')::date as day
  from bounds b
),
o_daily as (
...
fresh as (
  select
    ...
    -- last COMPLETE day GA4 has data for (today's partial row is not "fresh")
    (select max(date) from public.ga4_daily where date < current_date and sessions > 0) as ga4_last_day,
