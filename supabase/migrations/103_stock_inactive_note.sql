-- ============================================================
-- Migration 103: the reason a book is switched off travels with the flag.
--
-- The store export's deactivation_notes tells apart the ones that are
-- never coming back — مكرر (duplicate, 65), اصدار قديم (old edition,
-- 126), ترم اول 2025 منتهي (last year's term, 51), غير مخصص للبيع (not
-- for sale) — from the ones that are: اغلاق سعر (price closed, 114),
-- مغلق لحين الاعلان (closed until announced, 98), مؤقت (temporary, 22).
-- The Inactive tab needs that word to decide what to do with the
-- warehouse copies behind each SKU.
--
-- The store-side upload owns the note the same way it owns the flag: a
-- file that says active again clears the old reason. Seeded once by
-- hand from the 2026-08-16 export (603 notes); the Data Center keeps it
-- current from here.
--
-- Run after 102_stock_active_flag_and_surge.sql
-- ============================================================

alter table public.stock_items add column if not exists ecom_note text;
...
begin
  ...
  insert into public.stock_items (
    ...
    ecom_stock_at, sap_stock_at, ecom_active, ecom_note, updated_at
  )
  select distinct on (r->>'sku')
    ...
    case when p_side in ('ecom','both') then nullif(r->>'ecom_note','') end,
    now()
  ...
  on conflict (sku) do update set
    ...
    -- a store file that carries the flag also owns the note: when it says
    -- active again, the old reason goes
    ecom_note     = case when p_side in ('ecom','both') and excluded.ecom_active is not null
                         then excluded.ecom_note else stock_items.ecom_note end,
...
-- same signature as 102, so the drop is only for the return type
...
returns table (
  ...
  -- new in 103: why the store switched it off
  ecom_note text
)
