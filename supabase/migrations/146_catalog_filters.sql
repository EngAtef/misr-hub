-- ============================================================
-- Migration 146: Products page filters.
--
-- The catalog page could only narrow by search text + a scope chip.
-- It now filters by every catalog attribute the store export carries:
--   category    = store section (Kids / Cultural / Comics / Foreign / AL-Adwaa)
--   vendor      = brand (DISNEY, NATIONAL GEOGRAPHIC, Authored Kids Books ...)
--   subcategory = products.category (Stories & Adventures, Islamic Books ...)
--   publisher, author, language, age, series, cover_type, semester
--
-- All of them travel in ONE jsonb parameter so the RPC signature stays
-- stable as filters are added:  {"category":["Kids"],"vendor":["DISNEY"]}
-- An absent or empty key means "all". Options for the dropdowns come
-- from fn_catalog_filter_options (products + stock_items, no sales scan,
-- so it is cheap to call on page load).
--
-- fn_catalog_products / _totals are dropped and recreated: the return
-- type gains `subcategory` and both gain `p_filters jsonb` as the LAST
-- parameter, so every existing call keeps working unchanged.
-- ============================================================

-- jsonb {"key": ["a","b"]} -> text[] ; null when absent / not an array / empty.
-- coalesce() guards the jsonb_typeof(NULL) trap: a missing key would
-- otherwise yield NULL, fall to the else branch and return an empty array
-- (which matches nothing) instead of NULL (which means "no filter").
create or replace function public.fn_jsonb_text_array(p jsonb, p_key text)
returns text[]
language sql immutable
as $$
  select case
    when p is null
      or coalesce(jsonb_typeof(p -> p_key), '') <> 'array'
      or jsonb_array_length(p -> p_key) = 0
    then null
    else array(select jsonb_array_elements_text(p -> p_key))
  end;
$$;
...
-- ---- dropdown options: every distinct value per filter field + how many
-- catalog SKUs carry it. Same coalesce rules as the catalog RPC so a
-- chosen option always matches what the table shows.
