-- 123: follow-up to 122. With order months now cut at Cairo midnight, an order placed
-- 00:00-03:00 Cairo on the 1st has its GA4 transaction in the PREVIOUS month's
-- ga4_transactions sync (the GA4 property's timezone runs behind Cairo, so GA4 dates
-- those purchases to the last day of the prior month). Order-side tracking joins now
-- match transaction ids across the previous + current period_month; GA4-side aggregates
-- (GA4's own month totals) keep using period_month = the report month.
-- Verified Aug 2026: orders 23149-23163 are tracked (July period rows) — without this
-- they would have shown as 15 fake untracked orders.
...
-- one row per transaction matched to an order in this Cairo month; the GA4 row may
-- sit in the previous period_month (GA4 property tz runs behind Cairo)
tx as (
  select distinct on (t.transaction_id)
         t.transaction_id, t.campaign,
...
-- ---------------------------------------------------------------------------
-- tracked/untracked now match across previous + current period_month;
-- ga4_transactions / ga4_revenue stay GA4's own month; ga4_only (phantom
-- transactions) now checks against all orders instead of the month window
...
as $function$
  ...
  g as (
    ...
    from public.ga4_transactions t
    where t.period_month in ((p_month - interval '1 month')::date, p_month)
      and exists (select 1 from o where o.order_number = t.transaction_id)
  )
  select json_build_object(
    'orders', (select count(*) from o),
    'orders_revenue', (select coalesce(sum(total_order_amount),0) from o),
    'ga4_transactions', (select count(*) from g_month),
    'ga4_revenue', (select coalesce(sum(revenue),0) from g_month),
    ...
    'ga4_only', (select count(*) from g_month gm where not exists (select 1 from public.orders x where x.order_number = gm.transaction_id)),
    'untracked_by_source', (
...
-- ---------------------------------------------------------------------------
-- by_channel tracked counts now match across previous + current period_month
-- (txm CTE); everything else unchanged from 122
...
chan as (
  ...
  from o
  left join txm t on t.transaction_id = o.order_number
  group by 1
),
tx as (
  select t.*,
